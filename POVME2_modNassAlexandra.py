# This is a modified version of POVME 2.0.1
# It was modified 2017
# Modifications allow output of the complementary map compared to the one created for pocket volume calculations
#
...
class MultithreadingCalcVolumeTask(MultithreadingTaskGeneral):
	...
	#save points inside protein or ligand
	rememberedall = pts	
	
	# ... to here were addedto the original POVME2
...
#the lines from here...
	...
	#make lists from whole set and final pts set
	print (rememberedall)
	rememberedall_list = rememberedall.tolist()
	print (rememberedall_list)
	...
	# find not-shared points of both lists
	rest = tuple(x for x in rememberedall_list if x not in pts_list)
	print (rest)
	restarray = numpy.asarray(rest)



	# write the remembered points (those in protein or ligand) to .pdb
        if parameters['SaveIndividualPocketVolumes'] == True:
	    remem_text = ""
	   remem_text = remem_text + "REMARK Frame " + str(frame_indx) + "\n"
       remem_text = remem_text + numpy_to_pdb(restarray,'X')
	   remem_text = remem_text + "END\n"

	fl = open(parameters['OutputFilenamePrefix'] + "inLigOrProt_frame_" + str(frame_indx) + ".pdb", 'w')
	fl.write(remem_text)
	fl.close

	#... to here were added to the original POVME2
